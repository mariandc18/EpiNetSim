...
model = spreading_make_sird_model(pInfect=0.3, pRecover=0.05, pDeath=0.02)
...
for step in range(0, iterations, 20):
    ...
    count_d = sum(1 for i in er.nodes() if er.nodes[i]['state'] == SPREADING_DEAD)
    
    print(f"Iteration {step}: S={count_s}, I={count_i}, R={count_r}, D={count_d}")  
